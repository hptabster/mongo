...
var upsertedResult = function(query, expr) {
    ...
    result = coll.findAndModify({ query : query, update : expr, upsert : true, new : true });
    return result;
};

var upsertedField = function(query, expr, fieldName) {
    upsertedResult(query, expr);
...
assert.throws(function() { upsertedResult({ x : 1 }, {}); });
...
assert.throws(function() { upsertedResult({}, { $set : { a : 1, x : 1 } }); });
assert.throws(function() { upsertedResult({ x : { $gt : 1 } }, { $set : { a : 1, x : 1 } }); });
assert.throws(function() { upsertedResult({ x : { $in : [1] } }, { $set : { a : 1, x : 1 } }); });

// Shard key type errors
assert.throws(function() { upsertedResult({ x : undefined }, { $set : { a : 1 } }); });
assert.throws(function() { upsertedResult({ x : [1, 2] }, { $set : { a : 1 } }); });
assert.throws(function() { upsertedResult({ x : { $eq : { $gt : 5 } } }, { $set : { a : 1 } }); });
// Regex shard key is not extracted from queries, even exact matches
assert.throws(function() { upsertedResult({ x : { $eq : /abc/ } }, { $set : { a : 1 } }); });

// nested field extraction always fails with non-nested key - like _id, we require setting the
// elements directly
assert.throws(function() { upsertedResult({ "x.x" : 1 }, { $set : { a : 1 } }); });
assert.throws(function() { upsertedResult({ "x.x" : { $eq : 1 } }, { $set : { a : 1 } }); });
...
assert.throws(function() { upsertedResult({ "x.x" : [] }, { $set : { a : 1 } }); });
assert.throws(function() { upsertedResult({ x : { x : [] } }, { $set : { a : 1 } }); });
assert.throws(function() { upsertedResult({ x : [{ x : 1 }] }, { $set : { a : 1 } }); });

// Can't set sub-fields of nested key
assert.throws(function() { upsertedResult({ "x.x.x" : { $eq : 1 } }, { $set : { a : 1 } }); });
