...
// _id type error
assert.throws(function() { upsertedResult({}, { _id : [1, 2] }); });
assert.throws(function() { upsertedResult({}, { _id : undefined }); });
assert.throws(function() { upsertedResult({}, { $set : { _id : [1, 2] } }); });
// Fails in v2.6, no validation
assert.throws(function() { upsertedResult({}, { $setOnInsert : { _id : undefined } }); });
...
for (var i = 0; i < 3; i++) {
    ...
    assert.throws(function() { upsertedResult({ _id : [1, 2] }, expr); });
    assert.throws(function() { upsertedResult({ _id : undefined }, expr); });
    assert.throws(function() { upsertedResult({ _id : { $eq : [1, 2] } }, expr); });
    assert.throws(function() { upsertedResult({ _id : { $eq : undefined } }, expr); });
    assert.throws(function() { upsertedResult({ _id : { $all : [ 1, 2 ] } }, expr); });
    // All below fail in v2.6, non-_id fields completely ignored
    assert.throws(function() { upsertedResult({ $and : [{ _id : 1 }, { _id : 1 }] }, expr); });
    assert.throws(
        function() { upsertedResult({ $and : [{ _id : { $eq : 1 } }, { _id : 2 }] }, expr); });
    assert.throws(function() { upsertedResult({ _id : 1, "_id.x" : 1 }, expr); });
    assert.throws(function() { upsertedResult({ _id : { x : 1 }, "_id.x" : 1 }, expr); });

    // Special case - nested _id fields only used on $op-style updates
    if (isReplStyle) {
        // Fails in v2.6
        assert.throws(function() { upsertedResult({ "_id.x" : 1, "_id.y" : 2 }, expr); });
    }
    else {
        // Fails in v2.6
...
assert.throws(function() { upsertedResult({}, { _id : /abc/ }); });
// Fails in v2.6, no validation
assert.throws(function() { upsertedResult({}, { $set : { _id : /abc/ } }); });
...
for (var i = 0; i < 3; i++) {
    ...
    assert.throws(function() { upsertedResult({ x : undefined }, expr); });

    if (!isReplStyle) {
        assert.throws(function() { upsertedResult({ x : { 'x.x' : 1 } }, expr); });
        assert.throws(function() { upsertedResult({ x : { $all : [ 1, 2 ] } }, expr); });
        assert.throws(function() { upsertedResult({ $and : [{ x : 1 }, { x : 1 }] }, expr); });
        assert.throws(
            function() { upsertedResult({ $and : [{ x : { $eq : 1 } }, { x : 2 }] }, expr); });
    }
    else {
    ...
    if (!isReplStyle) {
        assert.throws(function() { upsertedResult({ x : 1, "x.x" : 1 }, expr); });
        assert.throws(function() { upsertedResult({ x : {}, "x.x" : 1 }, expr); });
        assert.throws(function() { upsertedResult({ x : { x : 1 }, "x.x" : 1 }, expr); });
        assert.throws(function() { upsertedResult({ x : { x : 1 }, "x.y" : 1 }, expr); });
        assert.throws(function() { upsertedResult({ x : [1, { x : 1 }], "x.x" : 1 }, expr); });
    }
    else {
